...
1. context_understanding: 
   ...
   - 기술적 제약(라이브러리 미지원 등)과 사용자의 실수를 명확히 구분할 것.
   - 이미 해결된 문제를 다시 시도하라는 조언은 하지 말 것.
...
3. feedback (작성 규칙을 반드시 따를 것):
   - 첫 문장: 사용자가 언급하지 않은 기술적 배경 설명 (왜 그런 현상/제약이 있는지).
   - 둘째 문장: 사용자의 접근법에 대한 구체적 평가.
   - 셋째 문장: 구체적인 API명이나 키워드를 포함한 다음 학습 제안.
   - 절대 금지: 사용자가 쓴 문장을 바꿔 말하기, 확인할 수 없는 기술 정보 작성.
   - 어투: '귀하', '~하십시오' 같은 격식체 금지. 편안한 존댓말 사용 (예: ~했네요, ~해보세요, ~거든요, ~이에요).
